...
const DistanceProgressBar = ({ distance, maxDistance, isInRange }) => {
  ...
  const safeMaxDistance = parseFloat(maxDistance) || 100;

  // Calculate progress percentage (0-100)
  const progressPercentage = useMemo(() => {
    if (safeMaxDistance === 0) return 0;
    const progress = Math.max(0, Math.min(100, ((safeMaxDistance - safeDistance) / safeMaxDistance) * 100));
    return parseFloat(progress) || 0;
  }, [safeDistance, safeMaxDistance]);
  ...
  const getColorForDistance = useMemo(() => {
    if (safeDistance <= 5) return COLORS.success;
    if (safeDistance <= 15) return COLORS.warning;
    return COLORS.error;
  }, [safeDistance]);
  ...
  const distanceText = useMemo(() => {
    ...
    if (safeDistance < 1000) return `${Math.round(safeDistance)}m remaining`;
    return `${parseFloat(safeDistance / 1000).toFixed(1)}km remaining`;
  }, [safeDistance, isInRange]);
